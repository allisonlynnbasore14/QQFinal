
var constants = module.exports = {
    DIR:{
    	MATCH: 'Match the definitions with their words.',
    	COPY: 'Type the whole statement into the textbox. Do not worry about spacing.',
    	STUDY: 'Copy the information.',
    	ANSWERQ: 'Answer the question with the single correct answer.',
    	PICTURE: 'Use the image to answer the question.',
    	ANGLE: 'Use the angles given to classify the type of triangle.',
    	PROPERTY: 'Choose an example of each property.',
    	FILL: 'Fill in the blanks to answer the questions.',
        FREE: 'Notify Mrs.Basore that you got a Problem Pass!',
    	//CLICK_PRIME: 'Use the prime number sieve to sift out all the composite and leave 25 prime numbers. Select the first prime number and then mentally discount all multiples composites of that number.  Continue this method with the first four prime numbers until all twenty-five prime numbers are selected. ',
        CLICK_PRIME: 'Select the first prime number and then mentally discount all multiples composites of that number.  Continue until all twenty-five prime numbers are selected.',
        LIST_PRIME: 'List all the prime numbers 100.',
    	LIST_SQUARE_NUMBERS: 'List the first 15 Square Numbers.',
    	COMP_PRIME: 'Click the composite numbers, but leave the prime numbers alone.',
    	DICE: 'For a regular dice, answer the questions about Probability and Odds.',
    	SEQUENCE: 'Determine if the sequence has a difference, ratio or neither.  Then find the 10th term and use the formulas if necessary.',
        HOME: 'Click Submit to start your next quarter quiz.',
        LOGIN: 'Type the name of the mathematician that was given to you and hit enter to get started!',
        LOGIN_ERROR: 'Sorry, the username was not found',
        KEY: 'Everytime a user sends a submission, this page will reset with their new scores. It will not record a test taken on the same day',
        DONE: 'Show Mrs.Basore this page. Nice Work!'
    },
    TITLE:{
        ANG: 'Angles',
        SEQ: 'Sequences',
        PRO: 'Properties',
        PRB: 'Probability',
        PIN: 'Prime Numbers',
        PER: 'Percents',
        PTH: 'Pythagorean Theorem ',
        FRE: 'Free Pass',
        FRA: 'Fraction',
        SQU: 'Square Numbers',
        EXP: 'Exponent',
        DRU: 'Division Rules',
        DON: 'Horray!',
        HOM: 'Welcome to Quarter Quizzes!',
        LOG: 'Login to QQ',
        KEY: 'Welcome Angela'
    },
    ORDER:{

        PER1: 1,
        PER2: 2,
        PER3: 3,
        PER4: 4,
        PER5: 5,

        PRO1: 6,
        PRO2: 7,
        PRO3: 8,
        PRO4: 9,
        PRO5: 10,

        FRE1: 11,

        DRU1: 12,
        DRU2: 13,
        DRU3: 14,

        PIN1: 15,
        PIN2: 16,
        PIN3: 17,
        PIN4: 18,
        PIN5: 19,

        EXP1: 20,
        EXP2: 21,
        EXP3: 22,
        EXP4: 23,
        EXP5: 24,

        FRA1: 25,
        FRA2: 26,
        FRA3: 27,
        FRA4: 28,
        FRA5: 29,
        FRA6: 30,

        FRE2: 31,

        SQU1: 32,
        SQU2: 33,
        SQU3: 34,
        SQU4: 35,
        SQU5: 36,

        PTH1: 37,
        PTH2: 38,
        PTH3: 39,

        ANG1: 40,
        ANG2: 41,
        ANG3: 42,
        ANG4: 43,
        ANG5: 44,

        FRE3: 45,

        PRB1: 46,
        PRB2: 47,
        PRB3: 48,
        PRB4: 49,
        PRB5: 50,

        SEQ1: 51,
        SEQ2: 52,
        SEQ3: 53,

        DON: 54,
    }
}
